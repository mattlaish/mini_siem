...
- Service-account hardening: `install-services.sh` creates `siem` with a
  ...
  enabling services.
- The installer now prefers repository `.venv/bin/python3`/`python`, then
  `venv/bin/python3`/`python`. If neither exists it bootstraps `.venv` and
  installs `requirements.txt`; system Python is bootstrap-only and is never the
  final systemd runtime. This also supports repair of deployments that already
  use `/opt/mini_siem/venv`.
- Re-running `install-services.sh` is the supported repair path for partial
  service state. It recreates both units and fails closed if either unit is not
  enabled/active, the unit is not pinned to the selected venv, or the expected
  listener ports are absent.
...
## PostgreSQL Migration Continuity Notes

Current engineering context:

- PostgreSQL migration investigation identified schema initialization and identity preservation as key risks.
- Do not continue from accumulated migration artifact packages.
- Maintain a controlled source baseline for implementation work.

Required implementation discipline:

1. Start from the selected source baseline.
2. Modify application source directly.
3. Keep tests aligned.
4. Update engineering documentation with actual implementation changes.
5. Deliver complete source artifacts only.
